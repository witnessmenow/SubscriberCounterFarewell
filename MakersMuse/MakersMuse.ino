/*******************************************************************
    Showing Makers Muse's sub count on a 64x64 RGB LED Matrix
...
// The Angus' Channel ID
#define CHANNEL_ID "UCxQbYGpbdrh-b2ND-AfIybg" // Makes up the channel URL
...
void animationHandler()
{
  ...
  if (!finishedAnimating) {
    ...
    drawImage(0, 4, image, LOGO_WIDTH, LOGO_HEIGHT);
    display.setTextColor(myMAGENTA);
    displayText(subCount, 50);
...
void setup() {
  ...
  display.print("MakersMuse");
  display.setCursor(2, 24);
  display.setTextColor(myWHITE);
  display.print("Empower");
  display.setCursor(2, 32);
  display.print("Creativity");
  display.setCursor(2, 40);
  display.print("Through");
  ...
  printer.setBounds(0, 40, 64, 4);
  printer.resetState();
  printer.headColour = myRED;
  ...
  printer.increments = 3; //1 is the default
}
